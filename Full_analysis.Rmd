...
```{r }
# Cargar datos
allprojects <- read.xlsx( xlsxFile = "Tables A tale of native american whole genome sequencing.xlsx", sheet = "Sup Table YYY", startRow = 2 )
...
# Save
ggsave( filename = "timepanelv2.png",
        # plot = timedot,
        width = 14,
        height = 7,
        dpi = 600 )
...
treemaper <- function( the_data, the_time, n_cut, the_color ) {
  ...
  pop_summary <- tmp_data %>% 
    group_by( Homologated_pop_code ) %>%
    summarise( samples = n( ) ) %>% 
    arrange( -samples ) %>% 
    ungroup( ) %>% 
    mutate( tag = ifelse( test = samples < sample_cutoff,
                          yes = "other",
                          no = Homologated_pop_code )) %>% 
    group_by( tag ) %>% 
...
studiedgroups <- full_natam_samples$Homologated_pop_code %>% unique( ) %>% sort( )
...
full_natam_samples %>% 
  select( Technology, Homologated_pop_code ) %>% 
  unique( ) %>% 
  group_by( Technology ) %>% 
...
allcensus <- read.xlsx(xlsxFile = "Tables A tale of native american whole genome sequencing.xlsx", sheet = "Sup Tabl 1.", startRow = 2)

# find the "censed" natam groups that have not been represented
notrepresented <- allcensus %>% 
  filter( !(allcensus$Homologated_pop_code %in% studiedgroups) ) %>% 
...
missing_heat <- ggplot( data = long %>% filter( groups > 0 ),
  ...
  scale_fill_gradient( low = "white",
                       high = "tomato" ) +
  labs( title = "NatAm groups not represented in genomics",
        x = "Population size",
        fill = "# of groups" ) +
  theme_light( ) +
  theme_void( ) +
...
ggsave( filename = "map_and_missing.png",
        width = 10,
        height = 5,
        dpi = 600 )
...
# read data
allcountries <- read.xlsx( xlsxFile = "Tables A tale of native american whole genome sequencing.xlsx", sheet = "Sup Tab 3", startRow = 2 )
...
# completepanel <- plot_grid( authorspanel,
#                             barplotparticipants,
#                             ncol = 1 )
...
# Save the panel
ggsave( filename = "authorship_panel.png",
        plot = authorspanel,
        width = 7,
        height = 7,
        dpi = 600 )
...
# Save the panel
ggsave( filename = "other_authors.png",
        plot = barplotparticipants,
        width = 7,
        height = 5,
        dpi = 600 )
```
